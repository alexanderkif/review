...
export const shiftColor = computed(
  () =>
    (color: string, multiply: number): string => {
...
export const getBgStyles = computed(() => (bg: BgStyles): string => {
...
export const getBgImage = computed(() => (bg: BgStyles): string => {
  const { image } = bg;
  if (image) return `background: url('/img/${image}');`;
  return '';
});

export const getGradientColor = computed(
  () =>
    (bg: BgStyles, number = 0): string => {
      const DEFAULT_TOP = '#999999';
      const DEFAULT_BOTTOM = '#eeeeee';

      return (
        bg?.gradient?.[number].color ||
        (number > 0 ? DEFAULT_TOP : DEFAULT_BOTTOM)
      );
    }
);
